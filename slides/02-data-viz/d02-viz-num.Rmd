...
- Second question to ask yourself is what are the variable types.
 ...
 - Numerical: 
      ...
      - can be further classified as **continuous** or **discrete** based on whether or not the variable can take on an infinite number of values or only whole numbers, respectively.
...
ggplot(loans, aes(x = annual_income, y = loan_amount)) +
  geom_point()
```
...
```{r warning = FALSE}
ggplot(loans, aes(x = annual_income, y=loan_amount, color = homeownership)) +
  geom_point()
```
---
